...
class MLXServerManager:
    ...
    def _create_mlx_command(self, instance: MLXServerProcess) -> List[str]:
        ...
        cmd = [
            ...
            "0.0.0.0",
            "--max-tokens",
            str(mlx_config["max_tokens"]),
            "--temperature",
            str(mlx_config["temperature"]),
            "--top-p",
            str(mlx_config["top_p"]),
        ]
    ...
    def _start_instance(self, instance: MLXServerProcess) -> bool:
        ...
        try:
            ...
            env = os.environ.copy()
            env["MLX_GPU_MEMORY_FRACTION"] = str(
                self.config["mlx_servers"].get("gpu_memory_fraction", 0.8)
            )
    ...
    async def _health_check_instance(self, instance: MLXServerProcess) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"MLX instance {instance.id} health check failed: {response.status_code}"
                )
                instance.healthy = False
                return False
            ...
            logger.error(
                f"Unexpected error in health check for instance {instance.id}: {e}"
            )
            instance.healthy = False
            return False
            ...
            try:
                ...
                for instance in self.processes:
                    ...
                    if healthy:
                        ...
                        logger.warning(
                            f"Restarting unhealthy MLX instance {instance.id}"
                        )
                ...
                logger.info(
                    f"Health check: {healthy_count}/{len(self.processes)} instances healthy"
                )
    ...
    def _initialize_instances(self):
        ...
        for i in range(num_instances):
            instance = MLXServerProcess(id=i, port=base_port + i)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        for instance in self.processes:
            instance_status = {
                ...
                "uptime": (
                    time.time() - instance.start_time if instance.start_time else 0
                ),
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "avg_uptime": (
                statistics.mean(
                    [
                        time.time() - inst.start_time
                        for inst in self.processes
                        if inst.start_time
                    ]
                )
                if any(inst.start_time for inst in self.processes)
                else 0
            ),
        }
    ...
    try:
        ...
        while True:
            ...
            if int(time.time()) % 60 == 0:
                ...
                logger.info(
                    f"Status: {status['healthy_instances']}/{status['total_instances']} instances healthy"
                )
