...
class OptimizedServerInstance:
    ...
    def update_performance_score(self):
        ...
        throughput_score = min(100, (self.metrics['throughput_tokens_per_sec'] / 1485) * 100)
        ...
        self.performance_score = (
            throughput_score * 0.4 + response_score * 0.25 + 
            memory_score * 0.2 + uptime_score * 0.15
        )
...
class EnhancedServerManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.performance_monitor_interval = config.get('performance_monitor_interval', 10)
    ...
    def _initialize_instances(self):
        ...
        for i in range(num_instances):
            instance = OptimizedServerInstance(
                id=i,
                port=base_port + i
            )
    ...
    async def _start_instance(self, instance: OptimizedServerInstance):
        ...
        try:
            ...
            logger.error(f"Instance {instance.id} failed to start within {startup_timeout} seconds")
                ...
                for instance in self.instances:
                    if instance.status == 'running':
                        if await self._check_instance_health(instance):
                            ...
                            logger.warning(f"Instance {instance.id} is unhealthy, attempting restart")
                            await self._restart_instance(instance)
                
                # Log health summary
                logger.info(f"Health check: {healthy_count}/{total_instances} instances healthy")
    ...
    async def _restart_instance(self, instance: OptimizedServerInstance):
        ...
        if instance.restart_count >= self.max_restart_attempts:
            logger.error(f"Instance {instance.id} exceeded max restart attempts ({self.max_restart_attempts})")
            instance.status = 'failed'
            return
        
        # Check restart cooldown
        if instance.last_heartbeat and (time.time() - instance.last_heartbeat) < self.restart_cooldown:
            logger.info(f"Instance {instance.id} in restart cooldown, waiting...")
            return
        
        try:
            instance.restart_count += 1
            logger.info(f"Restarting instance {instance.id} (attempt {instance.restart_count})")
    ...
    async def _stop_instance(self, instance: OptimizedServerInstance):
        ...
        try:
            if instance.process:
                logger.info(f"Stopping instance {instance.id} (PID: {instance.process.pid})")
            ...
            try:
                ...
                for instance in self.instances:
                    if instance.status == 'running' and instance.metrics:
                        total_throughput += instance.metrics.get('throughput_tokens_per_sec', 0)
                        total_memory += instance.metrics.get('memory_usage_mb', 0)
                        running_instances += 1
                
                # Update global metrics
                if running_instances > 0:
                    self.global_metrics['average_throughput'] = total_throughput / running_instances
                ...
                logger.info(f"Performance: {running_instances} running, "
    ...
    async def _optimize_performance(self):
        ...
        try:
            ...
            avg_score = sum(inst.performance_score for inst in running_instances) / len(running_instances)
            ...
            if avg_score < 70:  # Low performance
                ...
                sorted_instances = sorted(running_instances, key=lambda x: x.performance_score)
                ...
                for instance in low_performers:
                    if instance.performance_score < 50:
                        logger.info(f"Restarting low-performing instance {instance.id} (score: {instance.performance_score:.1f})")
                        await self._restart_instance(instance)
    ...
    def _update_global_metrics(self):
        ...
        try:
            ...
            if running_instances:
                ...
                total_requests = sum(inst.metrics.get('requests_processed', 0) for inst in running_instances)
                total_tokens = sum(inst.metrics.get('tokens_processed', 0) for inst in running_instances)
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            running_instances = [inst for inst in self.instances if inst.status == 'running']
            healthy_instances = [inst for inst in running_instances if inst.performance_score > 70]
            
            total_throughput = sum(inst.metrics.get('throughput_tokens_per_sec', 0) for inst in running_instances)
            avg_response_time = statistics.mean([
                inst.metrics.get('average_response_time', 0) for inst in running_instances
            ...
            return {
                ...
                'instances': {
                    ...
                    'failed': len([inst for inst in self.instances if inst.status == 'failed'])
                },
                'performance': {
                    'total_throughput_tokens_per_sec': total_throughput,
                    'average_response_time_ms': avg_response_time,
                    'target_throughput_tokens_per_sec': 1485,
                    'throughput_efficiency': (total_throughput / 1485) * 100 if total_throughput > 0 else 0,
                    'peak_throughput_tokens_per_sec': self.global_metrics['peak_throughput']
                },
...
async def create_enhanced_manager_app(config: Dict[str, Any]) -> web.Application:
    ...
    async def startup_tasks(app):
        ...
        app['performance_task'] = asyncio.create_task(manager.perform_performance_monitoring())
        app['manager'] = manager
