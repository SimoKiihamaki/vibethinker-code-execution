...
class OptimizedMLXInstance:
    ...
    def performance_score(self) -> float:
        ...
        return (throughput_score * 0.4 + response_score * 0.25 + 
                success_score * 0.2 + load_score * 0.15)
...
class TokenBatchProcessor:
    ...
    async def process_batch(self, instance: OptimizedMLXInstance) -> List[Dict[str, Any]]:
...
class OptimizedMLXLoadBalancer:
    ...
    async def _monitor_performance(self):
        ...
        while True:
            try:
                ...
                avg_throughput = statistics.mean(self.global_throughput) if self.global_throughput else 0
                
                logger.info(f"Performance: {healthy_instances}/{len(self.instances)} healthy, "
                ...
                for instance in self.instances:
                    if instance.total_requests > 10:
                        ...
                        if current_tps < 1000:  # Below target
                            processor.max_batch_size = min(10, processor.max_batch_size + 1)
                        elif current_tps > 2000:  # Above target
                            processor.max_batch_size = max(2, processor.max_batch_size - 1)
    ...
    def select_instance(self, algorithm: str = 'performance') -> Optional[OptimizedMLXInstance]:
        ...
        if algorithm == 'round_robin':
            ...
            return min(healthy_instances, key=lambda x: (x.last_used, -x.performance_score))
            ...
            return min(healthy_instances, key=lambda x: (x.average_response_time, -x.current_throughput))
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance': {
                ...
                'throughput_efficiency': (current_throughput / 1485) * 100 if avg_throughput > 0 else 0
            },
