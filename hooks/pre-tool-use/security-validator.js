...
/**
 * Convert security level to numeric score
 */
function getSecurityScore(level) {
  if (typeof level === 'number') {
    if (!Number.isInteger(level) || level < 0 || level > 4) {
      throw new Error(`Numeric security level out of range: ${level} (must be 0-4)`);
    }
    return level;
  }
  if (typeof level !== 'string') {
    throw new Error(`Invalid security level type: ${typeof level}`);
  }
...
function getSecurityLevel(score) {
  if (typeof score !== 'number' || !Number.isFinite(score) || score < 0) {
    throw new TypeError(`Invalid security score: ${score}`);
  }
...
/**
 * Helper to combine two security levels while preserving severities
 */
function maxSecurityLevel(levelA, levelB) {
  const maxScore = Math.max(getSecurityScore(levelA), getSecurityScore(levelB));
  return getSecurityLevel(maxScore);
}
...
async function validateOperation(toolName, toolInput) {
  ...
  if (toolName === 'Write' || toolName === 'Edit') {
    ...
    if (filePath) {
      ...
      result.securityLevel = maxSecurityLevel(result.securityLevel, fileValidation.securityLevel);
    }
  }
  ...
  if (toolName === 'DeleteFile') {
    ...
    if (filePath) {
      ...
      result.securityLevel = maxSecurityLevel(result.securityLevel, deleteValidation.securityLevel);
    }
  }
  ...
  if (sensitiveValidation.warnings.length > 0) {
    ...
    result.securityLevel = maxSecurityLevel(result.securityLevel, sensitiveValidation.securityLevel);
  }
...
function validateBashCommand(command) {
  ...
  for (const risky of riskyCommands) {
    if (command.includes(risky)) {
      ...
      result.securityLevel = maxSecurityLevel(result.securityLevel, 'high');
    }
  }
  ...
  for (const network of networkCommands) {
    if (command.includes(network)) {
      ...
      result.securityLevel = maxSecurityLevel(result.securityLevel, 'medium');
    }
  }
