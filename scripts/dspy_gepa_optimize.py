...
def metric(example, prediction, trace=None, pred_name=None, pred_trace=None):
    ...
    if missing:
        return 0.5
    fields_ok = (
        isinstance(obj.get("metrics"), dict)
...
def build_trainset():
    try:
        ...
        prompts = [
            json.dumps({"file": f"src/file_{i}.ts", "mode": "concise"})
            for i in range(5)
        ]
...
def configure_student_lm():
    ...
    model_name = os.getenv(
        "MLX_MODEL", "lmstudio-community/Qwen3-VL-2B-Thinking-MLX-8bit"
    )
    ...
    try:
        ...
        if h.status_code != 200:
            print(
                "MLX load balancer health check failed. Set MLX_BASE_URL or start MLX servers."
            )
            return False
        ...
        print("MLX load balancer connection timed out. Check if servers are running.")
        return False
    except Exception as e:
        print(f"Unexpected error connecting to MLX load balancer: {e}")
        return False
        ...
        def forward(self, prompt=None, messages=None, **kwargs):
            ...
            try:
                ...
                raise ConnectionError(
                    f"Failed to connect to MLX server at {self.base_url}"
                )
            except requests.exceptions.Timeout:
                raise TimeoutError(
                    f"Request to MLX server at {self.base_url} timed out"
                )
            except requests.exceptions.HTTPError as e:
                raise RuntimeError(f"HTTP error from MLX server: {e}")
            except Exception as e:
                raise RuntimeError(f"Error calling MLX server: {e}")

    student = MLXStudentLM(
        base_url, model_name, temperature=1.0, max_tokens=4096, timeout_ms=timeout_ms
    )
...
def run_gepa(auto_level: str):
    ...
    if not ok:
        print(
            "No student LM configured. Set OPENROUTER_API_KEY or configure dspy manually."
        )
        return
    ...
    if hasattr(compiled, "teleprompter") and hasattr(compiled.teleprompter, "program"):
        ...
        print(
            json.dumps(
                {"status": "ok", "optimized_prompt": str(prog)}, ensure_ascii=False
            )
        )
    elif best:
