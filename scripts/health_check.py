...
class HealthChecker:
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    if cmdline and any('mcp-server/dist/index.js' in str(arg) for arg in cmdline):
                        return {
                            "status": "healthy",
                            "pid": proc.info['pid'],
                            "name": proc.info['name'],
        ...
        try:
            ...
            if successful_results:
                ...
                avg_tokens_per_second = statistics.mean(
                    ...
                    for r in successful_results if r['response_time_ms'] > 0
                )
            else:
    ...
    async def run_full_check(self) -> Dict[str, any]:
        ...
        self.results = {
            ...
            "load_balancer": results[0] if not isinstance(results[0], Exception) else {"status": "error", "error": str(results[0])},
            "mlx_instances": results[1] if not isinstance(results[1], Exception) else [],
            ...
            "performance_test": results[4] if not isinstance(results[4], Exception) else {"status": "error", "error": str(results[4])}
        }
    ...
    def calculate_overall_health(self) -> str:
        ...
        checks = [
            ...
            len([inst for inst in self.results.get("mlx_instances", []) if inst.get("status") == "healthy"]) > 20,  # At least 20 healthy instances
            self.results.get("mcp_server", {}).get("status") in ["healthy", "unhealthy"],  # Not error
    ...
    def print_summary(self):
        ...
        }.get(overall_health, "\033[90m")  # Gray
        ...
        healthy_instances = len([inst for inst in instances if inst.get("status") == "healthy"])
        print(f"\nMLX Instances: {healthy_instances}/{len(instances)} healthy")
        
        if instances:
            response_times = [inst.get("response_time_ms") for inst in instances if inst.get("response_time_ms")]
            if response_times:
        ...
        if perf_result.get("status") == "completed":
            print(f"\nPerformance Test:")
            print(f"  Successful Requests: {perf_result.get('successful_requests', 0)}/{perf_result.get('total_requests', 0)}")
            if perf_result.get("avg_response_time_ms"):
                print(f"  Avg Response Time: {perf_result['avg_response_time_ms']:.1f}ms")
            if perf_result.get("avg_tokens_per_second"):
                print(f"  Avg Tokens/Second: {perf_result['avg_tokens_per_second']:.1f}")
